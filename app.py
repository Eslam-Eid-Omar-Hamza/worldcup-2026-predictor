...
T = {
    "en": {
        "title": "Match Predictor", "subtitle": "Live international football predictions",
        ...
        "updated": "Data updated to", "disclaimer": "Football is unpredictable — these are probabilities from past form, not certainties. Made for fun ⚽",
        "verdict_fav": "**{a} {fa}** are the favourites, but **{b} {fb}** can cause an upset.",
        "verdict_close": "This is a coin-flip — **{a} {fa}** and **{b} {fb}** are very evenly matched!",
        "pens_note": "High chance of penalties 🥅 — anything can happen in a shootout!",
    },
    "ar": {
        "title": "متنبّئ الماتشات", "subtitle": "توقّعات حيّة لكرة القدم الدولية",
        ...
        "updated": "الداتا محدّثة حتى", "disclaimer": "الكورة مبتتحسبش — دي احتمالات من الأداء السابق مش نتايج مؤكدة. اتعملت للتسلية ⚽",
        "verdict_fav": "**{a} {fa}** هو المرشّح، بس **{b} {fb}** ممكن يعمل مفاجأة.",
        "verdict_close": "الماتش ونص — **{a} {fa}** و **{b} {fb}** متقاربين جداً!",
...
def load_and_train():
    ...
    rows = []
    for t in teams:
        h = recent[recent["home_team"] == t]; a = recent[recent["away_team"] == t]
        g = len(h) + len(a)
        if g < 5: continue
...
def predict(S, avg, home, away, mg=10):
    ...
    goes_et=pd_; goes_pens=pd_*pde
    fh=ph+pd_*(phe+pde*0.5); fa=pa+pd_*(pae+pde*0.5)
    return lh,la,ph,pd_,pa,goes_et,goes_pens,fh,fa
...
.stButton>button {
  background: linear-gradient(90deg,#00b894,#00cec9); color:#003; font-weight:800;
  border:none; border-radius:12px; padding:.6rem 1rem; font-size:1.05rem;
}
.stButton>button:hover { filter:brightness(1.1); }
.verdict {
  background: rgba(255,255,255,.07); border-left:4px solid #00b894;
...
st.markdown(f"<h1 style='text-align:center;margin-bottom:0;'>⚽ {tr['title']}</h1>",unsafe_allow_html=True)
st.markdown(f"<p style='text-align:center;opacity:.7;margin-top:4px;'>{tr['subtitle']}</p>",unsafe_allow_html=True)
...
with col1:
    home=st.selectbox(tr["home"],teams,index=teams.index("Egypt") if "Egypt" in teams else 0,
                      format_func=lambda t: f"{get_flag(t)}  {t}")
with col2:
    away=st.selectbox(tr["away"],teams,index=teams.index("Argentina") if "Argentina" in teams else 1,
                      format_func=lambda t: f"{get_flag(t)}  {t}")
...
if st.button(tr["predict"],type="primary",use_container_width=True):
    if home==away:
        ...
        fh_flag, fa_flag = get_flag(home), get_flag(away)
        lh,la,ph,pd_,pa,g_et,g_pen,fin_h,fin_a=predict(S,avg,home,away)

        # verdict sentence
        diff=abs(ph-pa)
        if diff<0.08:
            verdict=tr["verdict_close"].format(a=home,fa=fh_flag,b=away,fb=fa_flag)
        else:
            fav,fav_f,und,und_f=(home,fh_flag,away,fa_flag) if ph>pa else (away,fa_flag,home,fh_flag)
            verdict=tr["verdict_fav"].format(a=fav,fa=fav_f,b=und,fb=und_f)
        st.markdown(f"<div class='verdict'>{verdict}</div>",unsafe_allow_html=True)

        # colored probability bar
        st.markdown(f"#### {tr['in90']}")
        seg=f"""<div class='bar-wrap'>
        <div class='bar-seg' style='width:{ph*100:.0f}%;background:#d63031;'>{ph*100:.0f}%</div>
        <div class='bar-seg' style='width:{pd_*100:.0f}%;background:#636e72;'>{pd_*100:.0f}%</div>
        <div class='bar-seg' style='width:{pa*100:.0f}%;background:#0984e3;'>{pa*100:.0f}%</div>
        </div>
        <div style='display:flex;justify-content:space-between;font-size:.9rem;opacity:.85;'>
        <span>{fh_flag} {home} {tr['win']}</span><span>{tr['draw']}</span><span>{away} {tr['win']} {fa_flag}</span>
        </div>"""
        st.markdown(seg,unsafe_allow_html=True)

        g1,g2=st.columns(2)
        g1.metric(f"{fh_flag} {home} · {tr['exp_goals']}",f"{lh:.2f}")
        g2.metric(f"{fa_flag} {away} · {tr['exp_goals']}",f"{la:.2f}")
        ...
        if knockout:
            ...
            if g_pen>0.15:
                st.info(tr["pens_note"])
            st.markdown(f"#### {tr['qualify']}")
            q1,q2=st.columns(2)
            q1.metric(f"{fh_flag} {home}",f"{fin_h*100:.0f}%")
            q2.metric(f"{fa_flag} {away}",f"{fin_a*100:.0f}%")

        st.markdown(f"#### {tr['strengths']}")
        for team,flg in [(home,fh_flag),(away,fa_flag)]:
            ...
            st.markdown(
                ...
                f"<b>{flg} {team}</b> &nbsp;·&nbsp; {tr['attack']}: <b>{atk:.2f}</b> ({slabel(tr,atk)}) "
